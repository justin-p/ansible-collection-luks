...
class ActionModule(RebootActionModule):
    ...
    DEFAULT_LUKS_ECHO_TO_CRYPTROOT_UNLOCK = False
        ...
        for name in names:
            ...
            if value is not None:
                try:
                    ...
                    raise AnsibleActionFail("parse %s" % name, orig_exc=e)
        return None
    ...
    def luks_ssh_private_key_file(self):
        return self._get_task_arg(
            "luks_ssh_private_key_file"
        ) or self._try_get_connection_option("ansible_ssh_private_key_file")
    ...
    def luks_ssh_executable(self):
        return (
            self._get_task_arg("luks_ssh_executable")
            or self._try_get_connection_option("ansible_ssh_executable")
            or self.DEFAULT_LUKS_SSH_EXECUTABLE
        )

    @property
    def luks_ssh_keygen_executable(self):
        return (
            self._get_task_arg("luks_ssh_keygen_executable")
            or self.DEFAULT_LUKS_SSH_KEYGEN_EXECUTABLE
        )

    @property
    def luks_ssh_add_executable(self):
        return (
            self._get_task_arg("luks_ssh_add_executable")
            or self.DEFAULT_LUKS_SSH_ADD_EXECUTABLE
        )

    @property
    def luks_ssh_add_timeout(self):
        return (
            self._get_task_arg_int("luks_ssh_add_timeout")
            or self.DEFAULT_LUKS_SSH_ADD_TIMEOUT
        )

    @property
    def luks_ssh_connect_timeout(self):
        return self._get_task_arg_int(
    ...
    def luks_ssh_timeout(self):
        return (
            self._get_task_arg_int(
                "luks_ssh_timeout", "reboot_timeout", "reboot_timeout_sec"
            )
            or self.DEFAULT_REBOOT_TIMEOUT
        )

    @property
    def luks_ssh_reconnect_timeout(self):
        return (
            self._get_task_arg_int("luks_ssh_reconnect_timeout")
            or self.DEFAULT_LUKS_SSH_RECONNECT_TIMEOUT
        )
    ...
    def luks_stop_retry_on_output(self):
        return (
            self._get_task_arg("luks_stop_retry_on_output")
            or self.DEFAULT_LUKS_STOP_RETRY_ON_OUTPUT
        )
    ...
    @property
    def luks_echo_to_cryptroot_unlock(self):
        # Cannot use "or" here to see if it's unset, as the data type is bool
        value = self._task.args.get("luks_echo_to_cryptroot_unlock")
        if value is None:
            return self.DEFAULT_LUKS_ECHO_TO_CRYPTROOT_UNLOCK
        return boolean(value)
    ...
    def get_luks_ssh_args(self):
        ...
        display.vvv(
            "{action}: SSH connection args for LUKS: {args}".format(
    ...
    def run_luks_ssh_prompt(self, distribution, action_kwargs=None):
        ...
        # If echo-to-cryptroot-unlock is enabled, pipe password to cryptroot-unlock
        if self.luks_echo_to_cryptroot_unlock:
            try:
                display.vvv(
                    "{action}: Using echo-to-pipe method to unlock cryptroot via SSH".format(
                        action=self._task.action
                    )
                )
                # Use echo to pipe the password to cryptroot-unlock
                unlock_command = f"echo -n {self.luks_password} | cryptroot-unlock"
                unlock_args = args + [unlock_command]
                result = subprocess.run(
                    unlock_args,
                    stdout=subprocess.PIPE,
                    stderr=subprocess.STDOUT,
                    text=True,
                    timeout=self.luks_ssh_timeout,
                )
                display.vvv(
                    "{action}: cryptroot-unlock result (return code: {code}):\n\t{output}".format(
                        action=self._task.action,
                ...
                if result.returncode == 0:
                    display.display(
                        "{action}: LUKS SSH unlock successful, output:\n\t{output}".format(
                            action=self._task.action,
                            output=result.stdout.replace("\n", "\n\t"),
                        )
                    )
                    ...
                    for substr in self.luks_stop_retry_on_output:
                        if substr.casefold() in output.casefold():
                            display.vvv(
                                "{action}: LUKS unlock disk-encryption via SSH prompt failed, known stop keywords found, output:\n\t{output}".format(
                                    action=self._task.action, output=output
                                )
                            )
                            raise StopRetryLoop(
                                subprocess.CalledProcessError(
                                    result.returncode, unlock_args, output
                                )
                            )

                    display.warning(
                        "{action}: LUKS unlock disk-encryption via SSH prompt failed, output:\n\t{output}".format(
                            action=self._task.action, output=output
                        )
                    )
                ...
                display.warning(
                    "{action}: cryptroot-unlock timed out".format(
                        action=self._task.action
                    )
                )
                raise
                ...
                if e.returncode == 255:
                    ...
                    display.warning(
                        "{action}: LUKS SSH connection fail (non-fatal, will attempt multiple times), output:\n\t{output}".format(
                            action=self._task.action,
            ...
            # Original behavior for non-manual cryptroot start
            try:
                display.vvv(
                    "{action}: Attempting LUKS SSH unlock via SSH exec".format(
                        action=self._task.action
                    )
                )
                result = subprocess.run(
                    ...
                    check=True,
                )  # raise error on non-0 exit code
                display.display(
                    "{action}: LUKS SSH unlock successful, output:\n\t{output}".format(
                        action=self._task.action,
                        output=result.stdout.replace("\n", "\n\t"),
                ...
                if e.returncode == 255:
                    ...
                    for substr in self.luks_stop_retry_on_output:
                        if substr.casefold() in output.casefold():
                            display.vvv(
                                "{action}: LUKS unlock disk-encryption via SSH prompt failed, known stop keywords founds, output:\n\t{output}".format(
                                    action=self._task.action, output=output
                                )
                            )
                            raise StopRetryLoop(e)
    ...
    def unlock_luks(self, distribution, previous_boot_time, task_vars):
        display.vvv(
            "{action}: post-reboot: starting LUKS unlock retry loop".format(
                action=self._task.action
            )
        )
        ...
        try:
            ...
            if not self.luks_manual_unlock_on_fail:
                display.vvv(
                    "{action}: LUKS unlock failed. Not asking for manual unlock because luks_manual_unlock_on_fail was false".format(
                        action=self._task.action
                    )
                )
                return fail_result
            ...
            display.warning(
                "{action}: LUKS unlock failed. Please unlock the host manually: {ansible_host} (timeout: {timeout} seconds)".format(
            ...
            try:
                ...
                display.error(
                    "{action}: Timed out waiting for you to unlock host manually: {ansible_host} (timeout: {timeout} seconds)".format(
                        action=self._task.action, ansible_host=hostname, timeout=timeout
                    )
                )
    ...
    def set_result_elapsed(self, result: dict, start: datetime):
        ...
        if start.tzinfo is None:
            # If start is timezone-naive, assume it's UTC
    ...
    def run_reboot(self, distribution, previous_boot_time, task_vars):
        ...
        if post_reboot_delay > 0:
            display.vvv(
                "{action}: waiting an additional {delay} seconds (post_reboot_delay)".format(
        ...
        if post_unlock_delay > 0:
            display.vvv(
                "{action}: waiting an additional {delay} seconds (post_unlock_delay)".format(
    ...
    def add_private_key_to_ssh_agent(self, private_key: str):
        args = [
            ...
            "-t",
            str(self.luks_ssh_add_timeout),
            "-",  # read from STDIN
        ]
        try:
            display.vvv(
                "{action}: Adding private key to ssh-agent via ssh-add".format(
                    action=self._task.action
                )
            )
            subprocess.run(
            ...
            raise RuntimeError(
                "Failed adding private key to ssh-agent via ssh-add, output:\n{output}".format(
                    output=e.output
                )
            ) from e
    ...
    def remove_public_key_from_ssh_agent(self, public_key: str):
        ...
        try:
            display.vvv(
                "{action}: Removing private key from ssh-agent via ssh-add".format(
                    action=self._task.action
                )
            )
            subprocess.run(
            ...
            raise RuntimeError(
                "Failed removing private key from ssh-agent via ssh-add, output:\n{output}".format(
                    output=e.output
                )
            ) from e
    ...
    def private_key_to_public_key(self, private_key: str):
        ...
        try:
            display.vvv(
                "{action}: Converting private key to public key via ssh-keygen".format(
                    action=self._task.action
                )
            )
            result = subprocess.run(
            ...
            raise RuntimeError(
                "Failed converting SSH private key to public key, output:\n{output}".format(
                    output=e.output
                )
            ) from e

    def ri_do_until_success_or_timeout(
        self, action, reboot_timeout, action_desc, distribution, action_kwargs=None
    ):
            ...
            try:
                ...
                if action_desc:
                    try:
                        ...
                        error = to_text(e)
                    last_error_msg = (
                        f"{self._task.action}: {action_desc} fail '{error}'"
                    )
    ...
    def cleanup(self, force=False):
        if self._has_added_key_to_ssh_agent:
            try:
                ...
                display.warning(
                    "{action}: Failed cleaning up SSH key from SSH-agent, error:\n{error}".format(
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        return self.run_reboot(distribution, previous_boot_time, task_vars)
